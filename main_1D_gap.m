...
ct = 500000;
Rtr = 6; %target data rate
C = double(eulergamma); %euler constant
 
%D_all = 50000; %overall area D_all X D_all
r_c = 10; % size of each small cluster
[Ptxmean,Ptxmean_LoS] = power_norm(ct,r_c,beta, d,eps);% need to do power normalization
Ptxmean=1;Ptxmean_LoS=1;

snrdbm = [0: 10 : 30];
lambda_vector = [0.001: 0.001: 0.006];%[0.2 0.1 0.05 0.01 0.001];
...
for isnr = 1: length(lambda_vector)
    ...
    for ict = 1 : ct
        ...
        %outage probability
        if rate_conv_case_ict(ict)>rate_pa_case_ict(ict)
            sum1 = sum1 + 1; 
        end

        %test
        z0 = z_user(id_ty);
        r = z_BS(id_next) - z_BS(id_ty);
        zI = z_user(id_next) - z_BS(id_next);
        if (zI^2+d^2)/(r^2+d^2)<=d^2/((zI+r)^2+d^2)%zI*(zI+r)*(zI^2+r*zI+2*d^2)<=0
            sum2 = sum2 + 1;
        end
        if zI>0
            ddd=1;
    ...
    gap_pa(isnr) = sum1/ct; 
    gap_test(isnr) = sum2/ct;
 
    %analytical results 
    x_K1 = 2*lambda*r_c;
    y_K1 = 4*lambda*d^2/r_c;
    temp1 = integral(@(t) exp(-x_K1.*t - y_K1./t), 1, Inf);
    gap_ana(isnr) = (1-exp(-2*lambda*r_c))/4/lambda/r_c...
        -sqrt(2)*d/r_c*...
        [ besselk(1, 4*sqrt(2)*lambda*d) - r_c/2/sqrt(2)/d*temp1];
    gap_app(isnr) = 2*lambda*d^2/r_c*(-log(4*lambda*d^2/r_c)-C+1);
end

%plot( snrdbm,rate_conv,snrdbm,rate_conv_case,snrdbm,rate_pa,snrdbm,rate_pa_case )
semilogy(lambda_vector,gap_pa,lambda_vector,gap_ana) 
